 ...
 * tags:
 *   name: Books
 *   description: Books API (from db.json)
 */
...
router.put("/api/books/:id", async (req, res) => {
  try {
    if (req.headers["authorization"] === "123456") {
      const otherbooks = req.app.db.data.books.filter(
        (book) => book.id !== req.params.id
      );
      const filteredbook = req.app.db.data.books.find(
        (book) => book.id === req.params.id
      );
      if (!filteredbook) {
        res.sendStatus(404);
...
router.delete("/api/books/:id", async (req, res) => {
  try {
    const filteredbook = req.app.db.data.books.filter(
      (book) => book.id === req.params.id
    );
    if (!filteredbook) {
      res.sendStatus(404);
