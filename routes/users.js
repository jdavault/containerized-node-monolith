 ...
 * tags:
 *   name: Users
 *   description: Users API (from db.json)
 */
...
router.get("/api/users/:id", async (req, res) => {
  try {
    //if (req.headers["authorization"] === "123456") {
    const { users } = req.app.db.data;
    const filteredbser = users.find((user) => user._id === req.params.id);
    if (!filteredbser) {
      res.sendStatus(404);
    }
...
router.put("/api/users/:id", async (req, res) => {
  try {
    if (req.headers["authorization"] === "123456") {
      const otherbsers = req.app.db.data.users.filter(
        (user) => user._id !== req.params.id
      );
      const filteredbser = req.app.db.data.users.find(
        (user) => user._id === req.params.id
      );
      if (!filteredbser) {
        res.sendStatus(404);
...
router.delete("/api/users/:id", async (req, res) => {
  try {
    const filteredbser = req.app.db.data.users.filter(
      (user) => user._id === req.params.id
    );
    if (!filteredbser) {
      res.sendStatus(404);
